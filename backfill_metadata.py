...
level1/level2 字段。本脚本从 path 反推 level1/level2，并从 title 解析
...
def parse_levels_from_path(path: str) -> tuple[str, str]:
    """从 categorized_reports/{level1}/{level2} 反推分类。"""
...
def main() -> None:
    ...
    for i, item in enumerate(items, start=1):
        level1, level2 = parse_levels_from_path(item["path"])
        author = cr.parse_author(item["title"])
        report_date = cr.parse_date(item["title"])
        priority = DEFAULT_PRIORITY

        print(f"[{i}/{len(items)}] {item['title']}")
        print(f"  → {level1} / {level2} ({priority}) [{author or '?'} | {report_date or '?'}]")

        if args.dry_run:
            filled += 1
            continue

        cr.update_metadata(item["media_id"], author, report_date, level1, level2, priority)
        filled += 1
